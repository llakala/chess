...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HQJ} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HQJ} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HQD} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HQD} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HPR} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HPR} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(HQN)) -> {ok, HQN} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(HPZ)) -> {ok, HPZ} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(HPV)) -> {ok, HPV} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(HQT), fun((HQT) -> json())) -> json().
...
-spec array(list(HQX), fun((HQX) -> json())) -> json().
