...
-type message(HTU) :: {message, HTU} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(HTV, HTW) :: {continue,
        HTW,
        gleam@option:option(gleam@erlang@process:selector(HTV))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(HTX, HTY) :: {ready, HTX, gleam@erlang@process:selector(HTY)} |
    {failed, binary()}.

-type self(HTZ, HUA) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        HTZ,
        gleam@erlang@process:subject(HUA),
        gleam@erlang@process:selector(message(HUA)),
        gleam@otp@system:debug_state(),
        fun((HUA, HTZ) -> next(HUA, HTZ))}.

-type spec(HUB, HUC) :: {spec,
        fun(() -> init_result(HUB, HUC)),
        integer(),
        fun((HUC, HUB) -> next(HUC, HUB))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(HUD) :: {ack,
        {ok, gleam@erlang@process:subject(HUD)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 178).
-spec continue(HUK) -> next(any(), HUK).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(HUO, HUP), gleam@erlang@process:selector(HUO)) -> next(HUO, HUP).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(HVA))) -> gleam@erlang@process:selector(message(HVA)).
...
-spec receive_message(self(any(), HUW)) -> message(HUW).
...
-spec init_selector(
    gleam@erlang@process:subject(IAA),
    gleam@erlang@process:selector(IAA)
) -> gleam@erlang@process:selector(message(IAA)).
...
-spec initialise_actor(
    spec(any(), HVR),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(HVR)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), HWE)) -> {ok, gleam@erlang@process:subject(HWE)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(HWK, fun((HWL, HWK) -> next(HWL, HWK))) -> {ok,
        gleam@erlang@process:subject(HWL)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(HWR), HWR) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(HWT),
    fun((gleam@erlang@process:subject(HWV)) -> HWT),
    integer()
) -> HWV.
