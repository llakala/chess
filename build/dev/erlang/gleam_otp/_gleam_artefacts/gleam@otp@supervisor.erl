...
-type spec(IKB, IKC) :: {spec,
        IKB,
        integer(),
        integer(),
        fun((children(IKB)) -> children(IKC))}.

-opaque children(IKD) :: {ready, starter(IKD)} | {failed, child_start_error()}.

-opaque child_spec(IKE, IKF, IKG) :: {child_spec,
        fun((IKF) -> {ok, gleam@erlang@process:subject(IKE)} |
            {error, gleam@otp@actor:start_error()}),
        fun((IKF, gleam@erlang@process:subject(IKE)) -> IKG)}.
...
-spec start_child(child_spec(any(), IKN, IKO), IKN) -> {ok, child(IKO)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    ILB,
    instruction(),
    child_spec(any(), ILB, ILD),
    child(ILD)
) -> {ok, {child(ILD), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(ILU), child_spec(any(), ILU, ILX)) -> children(ILX).
...
-spec add(children(IMC), child_spec(any(), IMC, IMF)) -> children(IMF).
...
-spec supervisor(
    fun((IMK) -> {ok, gleam@erlang@process:subject(IML)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(IML, IMK, IMK).
supervisor(Start) ->
...
-spec worker(
    fun((IMS) -> {ok, gleam@erlang@process:subject(IMT)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(IMT, IMS, IMS).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(INA, INB, any()),
    fun((INB, gleam@erlang@process:subject(INA)) -> INH)
) -> child_spec(INA, INB, INH).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), INM)) -> gleam@otp@actor:init_result(state(INM), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(INS)) -> gleam@otp@actor:next(message(), state(INS)).
...
-spec loop(message(), state(INX)) -> gleam@otp@actor:next(message(), state(INX)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
