...
-opaque task(IXK) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:subject(IXK)}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message2(IXL, IXM) :: {m2_from_subject1, IXL} |
    {m2_from_subject2, IXM} |
    m2_timeout.

-type message3(IXN, IXO, IXP) :: {m3_from_subject1, IXN} |
    {m3_from_subject2, IXO} |
    {m3_from_subject3, IXP} |
    m3_timeout.

-type message4(IXQ, IXR, IXS, IXT) :: {m4_from_subject1, IXQ} |
    {m4_from_subject2, IXR} |
    {m4_from_subject3, IXS} |
    {m4_from_subject4, IXT} |
    m4_timeout.

-type message(IXU) :: {message, integer(), IXU} | message_timeout.
...
-spec try_await(task(IXZ), integer()) -> {ok, IXZ} | {error, await_error()}.
...
-spec await(task(IYD), integer()) -> IYD.
await(Task, Timeout) ->
...
-spec await_forever(task(IYH)) -> IYH.
await_forever(Task) ->
    assert_owner(Task),
    Selector = begin
        _pipe = gleam_erlang_ffi:new_selector(),
...
-spec try_await2_loop(
    gleam@erlang@process:selector(message2(IYR, IYS)),
    gleam@option:option({ok, IYR} | {error, await_error()}),
    gleam@option:option({ok, IYS} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, IYR} | {error, await_error()}, {ok, IYS} | {error, await_error()}}.
...
-spec try_await2(task(IYJ), task(IYL), integer()) -> {{ok, IYJ} |
        {error, await_error()},
    {ok, IYL} | {error, await_error()}}.
...
-spec try_await3_loop(
    gleam@erlang@process:selector(message3(IZS, IZT, IZU)),
    gleam@option:option({ok, IZS} | {error, await_error()}),
    gleam@option:option({ok, IZT} | {error, await_error()}),
    gleam@option:option({ok, IZU} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, IZS} | {error, await_error()},
    {ok, IZT} | {error, await_error()},
    {ok, IZU} | {error, await_error()}}.
...
-spec try_await3(task(IZG), task(IZI), task(IZK), integer()) -> {{ok, IZG} |
        {error, await_error()},
    {ok, IZI} | {error, await_error()},
    {ok, IZK} | {error, await_error()}}.
...
-spec try_await4_loop(
    gleam@erlang@process:selector(message4(JBE, JBF, JBG, JBH)),
    gleam@option:option({ok, JBE} | {error, await_error()}),
    gleam@option:option({ok, JBF} | {error, await_error()}),
    gleam@option:option({ok, JBG} | {error, await_error()}),
    gleam@option:option({ok, JBH} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, JBE} | {error, await_error()},
    {ok, JBF} | {error, await_error()},
    {ok, JBG} | {error, await_error()},
    {ok, JBH} | {error, await_error()}}.
...
-spec try_await4(task(JAO), task(JAQ), task(JAS), task(JAU), integer()) -> {{ok,
            JAO} |
        {error, await_error()},
    {ok, JAQ} | {error, await_error()},
    {ok, JAS} | {error, await_error()},
    {ok, JAU} | {error, await_error()}}.
...
-spec dict_to_list(gleam@dict:dict(integer(), JCX), integer(), JCX) -> list(JCX).
...
-spec try_await_all_loop(
    gleam@dict:dict(integer(), {ok, JCN} | {error, await_error()}),
    integer(),
    gleam@erlang@process:timer(),
    gleam@erlang@process:selector(message(JCN))
) -> list({ok, JCN} | {error, await_error()}).
...
-spec try_await_all(list(task(JCH)), integer()) -> list({ok, JCH} |
    {error, await_error()}).
