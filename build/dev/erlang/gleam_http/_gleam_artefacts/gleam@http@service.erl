...
-spec map_response_body(
    fun((HNN) -> gleam@http@response:response(HMZ)),
    fun((HMZ) -> HNA)
) -> fun((HNN) -> gleam@http@response:response(HNA)).
...
-spec prepend_response_header(
    fun((HNR) -> gleam@http@response:response(HNT)),
    binary(),
    binary()
) -> fun((HNR) -> gleam@http@response:response(HNT)).
...
-spec ensure_post(gleam@http@request:request(HNE)) -> {ok,
        gleam@http@request:request(HNE)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(HPA)) -> HPI)) -> fun((gleam@http@request:request(HPA)) -> HPI).
