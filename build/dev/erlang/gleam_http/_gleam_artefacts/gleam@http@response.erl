...
-type response(HIY) :: {response, integer(), list({binary(), binary()}), HIY}.
...
-spec set_header(response(HJN), binary(), binary()) -> response(HJN).
...
-spec prepend_header(response(HJQ), binary(), binary()) -> response(HJQ).
...
-spec set_body(response(any()), HJV) -> response(HJV).
...
-spec try_map(response(HIZ), fun((HIZ) -> {ok, HJB} | {error, HJC})) -> {ok,
        response(HJB)} |
    {error, HJC}.
...
-spec map(response(HJX), fun((HJX) -> HJZ)) -> response(HJZ).
...
-spec expire_cookie(response(HKH), binary(), gleam@http@cookie:attributes()) -> response(HKH).
