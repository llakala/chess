...
-spec set_header(request(GYX), binary(), binary()) -> request(GYX).
...
-spec prepend_header(request(GZA), binary(), binary()) -> request(GZA).
...
-spec set_body(request(any()), GZF) -> request(GZF).
set_body(Req, Body) ->
...
-spec map(request(GZH), fun((GZH) -> GZJ)) -> request(GZJ).
...
-spec set_query(request(GZT), list({binary(), binary()})) -> request(GZT).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(GZX), gleam@http:method()) -> request(GZX).
...
-spec set_scheme(request(HAE), gleam@http:scheme()) -> request(HAE).
...
-spec set_host(request(HAH), binary()) -> request(HAH).
set_host(Req, Host) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_port(request(HAK), integer()) -> request(HAK).
set_port(Req, Port) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_path(request(HAN), binary()) -> request(HAN).
set_path(Req, Path) ->
    _record = Req,
    {request,
        erlang:element(2, _record),
...
-spec set_cookie(request(HAQ), binary(), binary()) -> request(HAQ).
...
-spec remove_cookie(request(HAV), binary()) -> request(HAV).
