...
-opaque builder(GWW, GWX) :: {builder,
        integer(),
        fun((gleam@http@request:request(GWW)) -> gleam@http@response:response(GWX)),
...
-type websocket_message(GWY) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, GWY}.
...
-spec new(
    fun((gleam@http@request:request(GXV)) -> gleam@http@response:response(GXX))
) -> builder(GXV, GXX).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(GYB, GYC), integer()) -> builder(GYB, GYC).
port(Builder, Port) ->
    _record = Builder,
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), GYH),
    integer(),
    gleam@http@response:response(GYH)
) -> builder(mist@internal@http:connection(), GYH).
...
-spec bind(builder(GYT, GYU), binary()) -> builder(GYT, GYU).
...
-spec with_ipv6(builder(GYZ, GZA)) -> builder(GYZ, GZA).
with_ipv6(Builder) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(HAA)
) -> {ok, websocket_message(HAA)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((HAG, mist@internal@websocket:websocket_connection(), websocket_message(HAH)) -> gleam@otp@actor:next(HAH, HAG)),
    fun((mist@internal@websocket:websocket_connection()) -> {HAG,
        gleam@option:option(gleam@erlang@process:selector(HAH))}),
    fun((HAG) -> nil)
...
-spec server_sent_events(
    ...
    fun(() -> gleam@otp@actor:init_result(HAV, HAW)),
    fun((HAW, sse_connection(), HAV) -> gleam@otp@actor:next(HAW, HAV))
) -> gleam@http@response:response(response_data()).
