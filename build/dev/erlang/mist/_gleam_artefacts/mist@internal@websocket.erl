...
-type valid_message(GJZ) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, GJZ}.

-type websocket_message(GKA) :: {valid, valid_message(GKA)} | invalid.
...
-type handler_message(GKB) :: {internal, gramps@websocket:frame()} | {user, GKB}.

-type websocket_state(GKC) :: {websocket_state,
        bitstring(),
        GKC,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(GLK))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(GLK))).
...
-spec apply_frames(
    ...
    fun((GLA, websocket_connection(), handler_message(GLB)) -> gleam@otp@actor:next(GLB, GLA)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(GLB), GLA),
    fun((GLA) -> nil)
) -> gleam@otp@actor:next(websocket_message(GLB), GLA).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {GKL,
        gleam@option:option(gleam@erlang@process:selector(GKM))}),
    fun((GKL) -> nil),
    fun((GKL, websocket_connection(), handler_message(GKM)) -> gleam@otp@actor:next(GKM, GKL)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(GKM))} | {error, nil}.
