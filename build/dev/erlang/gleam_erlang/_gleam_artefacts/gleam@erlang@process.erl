...
-opaque subject(FVV) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FVV}.

-type do_not_leak() :: any().

-type selector(FVW) :: any() | {gleam_phantom, FVW}.
...
-type call_error(FVX) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FVX}.
...
-spec 'receive'(subject(FWI), integer()) -> {ok, FWI} | {error, nil}.
...
-spec receive_forever(subject(FWM)) -> FWM.
...
-spec select(selector(FWQ), integer()) -> {ok, FWQ} | {error, nil}.
select(From, Within) ->
...
-spec select_forever(selector(FWU)) -> FWU.
select_forever(From) ->
...
-spec map_selector(selector(FWW), fun((FWW) -> FWY)) -> selector(FWY).
map_selector(A, B) ->
...
-spec merge_selector(selector(FXA), selector(FXA)) -> selector(FXA).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(FXE), fun((exit_message()) -> FXE)) -> selector(FXE).
...
-spec selecting(selector(FXH), subject(FXJ), fun((FXJ) -> FXH)) -> selector(FXH).
...
-spec selecting_record2(
    selector(FXR),
    any(),
    fun((gleam@dynamic:dynamic_()) -> FXR)
) -> selector(FXR).
...
-spec selecting_record3(
    selector(FXV),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FXV)
) -> selector(FXV).
...
-spec selecting_record4(
    selector(FXZ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FXZ)
) -> selector(FXZ).
...
-spec selecting_record5(
    selector(FYD),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FYD)
) -> selector(FYD).
...
-spec selecting_record6(
    selector(FYH),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FYH)
) -> selector(FYH).
...
-spec selecting_record7(
    selector(FYL),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FYL)
) -> selector(FYL).
...
-spec selecting_record8(
    selector(FYP),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FYP)
) -> selector(FYP).
...
-spec selecting_anything(selector(FYT), fun((gleam@dynamic:dynamic_()) -> FYT)) -> selector(FYT).
...
-spec deselecting(selector(FXM), subject(any())) -> selector(FXM).
...
-spec deselecting_process_down(selector(FZI), process_monitor()) -> selector(FZI).
...
-spec try_call(subject(FZL), fun((subject(FZN)) -> FZL), integer()) -> {ok, FZN} |
    {error, call_error(FZN)}.
...
-spec call(subject(FZS), fun((subject(FZU)) -> FZS), integer()) -> FZU.
...
-spec try_call_forever(subject(GAA), fun((subject(GAC)) -> GAA)) -> {ok, GAC} |
...
-spec call_forever(subject(FZW), fun((subject(FZY)) -> FZW)) -> FZY.
...
-spec send_after(subject(GAJ), integer(), GAJ) -> timer().
