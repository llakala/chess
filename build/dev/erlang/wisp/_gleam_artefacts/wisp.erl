...
-spec method_override(gleam@http@request:request(HXM)) -> gleam@http@request:request(HXM).
...
-spec fn_with_bad_request_error(fun((HYN) -> {ok, HYO} | {error, any()})) -> fun((HYN) -> {ok,
        HYO} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((HYH, bitstring()) -> {ok, HYH} |
        {error, gleam@http@response:response(body())}),
    HYH
) -> {ok, {gleam@option:option(buffered_reader()), integer(), HYH}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), HZF})) -> list({binary(), HZF}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, HZI} | {error, any()},
    fun((HZI) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, HXZ} | {error, any()}) -> {ok, HXZ} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
