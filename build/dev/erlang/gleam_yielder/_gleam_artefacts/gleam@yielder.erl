...
-type action(JPG) :: stop | {continue, JPG, fun(() -> action(JPG))}.

-opaque yielder(JPH) :: {yielder, fun(() -> action(JPH))}.

-type step(JPI, JPJ) :: {next, JPI, JPJ} | done.

-type chunk(JPK, JPL) :: {another_by,
        list(JPK),
        JPL,
        JPK,
        fun(() -> action(JPK))} |
...
-type sized_chunk(JPM) :: {another, list(JPM), fun(() -> action(JPM))} |
    {last, list(JPM)} |
    no_more.
...
-spec unfold_loop(JPU, fun((JPU) -> step(JPV, JPU))) -> fun(() -> action(JPV)).
...
-spec unfold(JPP, fun((JPP) -> step(JPQ, JPP))) -> yielder(JPQ).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> JPZ)) -> yielder(JPZ).
repeatedly(F) ->
...
-spec from_list(list(JQD)) -> yielder(JQD).
from_list(List) ->
...
-spec transform(yielder(JQN), JQP, fun((JQP, JQN) -> step(JQQ, JQP))) -> yielder(JQQ).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(JQX)), fun((JQZ, JQX) -> JQZ), JQZ) -> JQZ.
...
-spec fold(yielder(JQU), JQW, fun((JQW, JQU) -> JQW)) -> JQW.
...
-spec to_list(yielder(JRC)) -> list(JRC).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(JRF)) -> step(JRF, yielder(JRF)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(JRN)), integer()) -> fun(() -> action(JRN)).
...
-spec take(yielder(JRK), integer()) -> yielder(JRK).
...
-spec drop_loop(fun(() -> action(JRT)), integer()) -> action(JRT).
...
-spec map(yielder(JRW), fun((JRW) -> JRY)) -> yielder(JRY).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(JSE), yielder(JSG), fun((JSE, JSG) -> JSI)) -> yielder(JSI).
...
-spec append_loop(fun(() -> action(JSU)), fun(() -> action(JSU))) -> action(JSU).
...
-spec append(yielder(JSQ), yielder(JSQ)) -> yielder(JSQ).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(JTC)))) -> action(JTC).
...
-spec flatten(yielder(yielder(JSY))) -> yielder(JSY).
flatten(Yielder) ->
...
-spec concat(list(yielder(JTG))) -> yielder(JTG).
concat(Yielders) ->
...
-spec flat_map(yielder(JTK), fun((JTK) -> yielder(JTM))) -> yielder(JTM).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(JTS)), fun((JTS) -> boolean())) -> action(JTS).
...
-spec filter(yielder(JTP), fun((JTP) -> boolean())) -> yielder(JTP).
...
-spec filter_map(yielder(JTV), fun((JTV) -> {ok, JTX} | {error, any()})) -> yielder(JTX).
...
-spec cycle(yielder(JUJ)) -> yielder(JUJ).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(JUR)), fun((JUR) -> boolean())) -> {ok, JUR} |
    {error, nil}.
...
-spec find(yielder(JUN), fun((JUN) -> boolean())) -> {ok, JUN} | {error, nil}.
...
-spec find_map(yielder(JUV), fun((JUV) -> {ok, JUX} | {error, any()})) -> {ok,
        JUX} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(JVO)), integer()) -> fun(() -> action({JVO,
    integer()})).
...
-spec index(yielder(JVL)) -> yielder({JVL, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(JVR, fun((JVR) -> JVR)) -> yielder(JVR).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(JVW)), fun((JVW) -> boolean())) -> fun(() -> action(JVW)).
...
-spec take_while(yielder(JVT), fun((JVT) -> boolean())) -> yielder(JVT).
...
-spec drop_while_loop(fun(() -> action(JWC)), fun((JWC) -> boolean())) -> action(JWC).
...
-spec drop_while(yielder(JVZ), fun((JVZ) -> boolean())) -> yielder(JVZ).
...
-spec scan_loop(fun(() -> action(JWJ)), fun((JWL, JWJ) -> JWL), JWL) -> fun(() -> action(JWL)).
...
-spec scan(yielder(JWF), JWH, fun((JWH, JWF) -> JWH)) -> yielder(JWH).
...
-spec zip_loop(fun(() -> action(JWS)), fun(() -> action(JWU))) -> fun(() -> action({JWS,
    JWU})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(JWN), yielder(JWP)) -> yielder({JWN, JWP}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(JXH)), fun((JXH) -> JXJ), JXJ, list(JXH)) -> chunk(JXH, JXJ).
...
-spec chunk_loop(fun(() -> action(JXC)), fun((JXC) -> JXE), JXE, JXC) -> action(list(JXC)).
...
-spec chunk(yielder(JWX), fun((JWX) -> any())) -> yielder(list(JWX)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(JXV)), integer(), list(JXV)) -> sized_chunk(JXV).
...
-spec sized_chunk_loop(fun(() -> action(JXR)), integer()) -> fun(() -> action(list(JXR))).
...
-spec sized_chunk(yielder(JXN), integer()) -> yielder(list(JXN)).
...
-spec intersperse_loop(fun(() -> action(JYC)), JYC) -> action(JYC).
...
-spec intersperse(yielder(JXZ), JXZ) -> yielder(JXZ).
...
-spec any_loop(fun(() -> action(JYH)), fun((JYH) -> boolean())) -> boolean().
...
-spec any(yielder(JYF), fun((JYF) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(JYL)), fun((JYL) -> boolean())) -> boolean().
...
-spec all(yielder(JYJ), fun((JYJ) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(JZB) -> fun((gleam@option:option(list(JZB))) -> list(JZB)).
...
-spec group_updater(fun((JYT) -> JYU)) -> fun((gleam@dict:dict(JYU, list(JYT)), JYT) -> gleam@dict:dict(JYU, list(JYT))).
group_updater(F) ->
...
-spec group(yielder(JYN), fun((JYN) -> JYP)) -> gleam@dict:dict(JYP, list(JYN)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(JZF), fun((JZF, JZF) -> JZF)) -> {ok, JZF} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(JZJ)) -> {ok, JZJ} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(JZR) -> yielder(JZR).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(JZX)), fun(() -> action(JZX))) -> action(JZX).
...
-spec interleave(yielder(JZT), yielder(JZT)) -> yielder(JZT).
...
-spec fold_until_loop(
    fun(() -> action(KAF)),
    fun((KAH, KAF) -> gleam@list:continue_or_stop(KAH)),
    KAH
) -> KAH.
...
-spec fold_until(
    yielder(KAB),
    KAD,
    fun((KAD, KAB) -> gleam@list:continue_or_stop(KAD))
) -> KAD.
...
-spec try_fold(yielder(KAJ), KAL, fun((KAL, KAJ) -> {ok, KAL} | {error, KAM})) -> {ok,
        KAL} |
    {error, KAM}.
...
-spec first(yielder(KAZ)) -> {ok, KAZ} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(KBD), integer()) -> {ok, KBD} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(KBL), fun((KBL) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(KBO, fun(() -> yielder(KBO))) -> yielder(KBO).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(KBR), KBR) -> yielder(KBR).
