...
-type message(BXD) :: {packet, bitstring()} | {user, BXD}.
...
-type connection(BXE) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(BXE))}.

-opaque handler(BXF, BXG) :: {handler,
        glisten@socket@options:interface(),
        fun((connection(BXF)) -> {BXG,
            gleam@option:option(gleam@erlang@process:selector(BXF))}),
        fun((message(BXF), BXG, connection(BXF)) -> gleam@otp@actor:next(message(BXF), BXG)),
        gleam@option:option(fun((BXG) -> nil)),
        integer(),
        boolean(),
        boolean()}.
...
-spec convert_on_init(
    fun((connection(BYU)) -> {BYW,
        gleam@option:option(gleam@erlang@process:selector(BYU))})
) -> fun((glisten@internal@handler:connection(BYU)) -> {BYW,
    gleam@option:option(gleam@erlang@process:selector(BYU))}).
...
-spec handler(
    fun((connection(BZC)) -> {BZE,
        gleam@option:option(gleam@erlang@process:selector(BZC))}),
    fun((message(BZC), BZE, connection(BZC)) -> gleam@otp@actor:next(message(BZC), BZE))
) -> handler(BZC, BZE).
handler(On_init, Loop) ->
    {handler, loopback, On_init, Loop, none, 10, false, false}.

-file("src/glisten.gleam", 188).
-spec map_user_selector(gleam@erlang@process:selector(message(BYJ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(BYJ)).
...
-spec convert_loop(
    fun((message(BYO), BYP, connection(BYO)) -> gleam@otp@actor:next(message(BYO), BYP))
) -> fun((glisten@internal@handler:loop_message(BYO), BYP, glisten@internal@handler:connection(BYO)) -> gleam@otp@actor:next(glisten@internal@handler:loop_message(BYO), BYP)).
convert_loop(Loop) ->
    fun(Msg, Data, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec with_close(handler(BZL, BZM), fun((BZM) -> nil)) -> handler(BZL, BZM).
...
-spec with_pool_size(handler(BZR, BZS), integer()) -> handler(BZR, BZS).
...
-spec with_http2(handler(BZX, BZY)) -> handler(BZX, BZY).
with_http2(Handler) ->
    _record = Handler,
    {handler,
        erlang:element(2, _record),
...
-spec with_ipv6(handler(CAJ, CAK)) -> handler(CAJ, CAK).
with_ipv6(Handler) ->
    _record = Handler,
    {handler,
        erlang:element(2, _record),
...
-spec bind(handler(CAD, CAE), binary()) -> handler(CAD, CAE).
