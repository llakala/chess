...
-type message(BED) :: {internal, internal_message()} | {user, BED}.

-type loop_message(BEE) :: {packet, bitstring()} | {custom, BEE}.

-type loop_state(BEF, BEG) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(BEF)),
        glisten@transport:transport(),
        BEG}.

-type connection(BEH) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(BEH))}.

-type handler(BEI, BEJ) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(BEI), BEJ, connection(BEI)) -> gleam@otp@actor:next(loop_message(BEI), BEJ)),
        fun((connection(BEI)) -> {BEJ,
            gleam@option:option(gleam@erlang@process:selector(BEI))}),
        gleam@option:option(fun((BEJ) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 73).
?DOC(false).
-spec start(handler(BET, any())) -> {ok,
        gleam@erlang@process:subject(message(BET))} |
