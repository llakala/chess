...
-spec method_override(gleam@http@request:request(SEL)) -> gleam@http@request:request(SEL).
...
-spec fn_with_bad_request_error(fun((SFM) -> {ok, SFN} | {error, any()})) -> fun((SFM) -> {ok,
        SFN} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((SFG, bitstring()) -> {ok, SFG} |
        {error, gleam@http@response:response(body())}),
    SFG
) -> {ok, {gleam@option:option(buffered_reader()), integer(), SFG}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), SGE})) -> list({binary(), SGE}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, SGH} | {error, any()},
    fun((SGH) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, SEY} | {error, any()}) -> {ok, SEY} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
