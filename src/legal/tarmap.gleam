...
/// This also sorts the moves before returning them, to give a deterministic
/// order. If you don't like the order, sort it yourself!
pub fn collection_to_moves(tarmaps: TarmapCollection) -> List(Move) {
  dict.fold(tarmaps, [], fn(accum, origin, targets) {
    let moves =
